...
void verificare_comanda(void *arg)
{
    ...
    while(1)
    {
        ...
        if(strcmp(comanda, "inregistrare") == 0) ///////////////// comanda executata pentru crearea unui cont
        {
            do{
                ...
                //verificare daca numele de utilizator poate fi folosit
                memset(raspuns, 0, sizeof(raspuns));
        ...
        } else if(strcmp(comanda, "conectare") == 0 && verificare_status(nume_utilizator) == false)
        {
            ...
            do{
                ...
                // aici va urma implementarea optiunilor urmatoare
                // 1. schimbare parola
                // 2. crearea unui cont
                // 3. iesirea din aplicatie
        ...
        } else if(strcmp(comanda, "trimite mesaj") == 0)
        {
            ...
            /// de verificat daca utilizatorul este online -> daca da se stocheaza in tabela mesaje mesajuln-> se va indexa mesajul cu id -ul sau 
...
int callback3(void *data, int argc, char **argv, char **azColName) 
{
    for (int i = 0; i < argc; i++) 
    {
        if(argv[i] != NULL)
        {
           ...
           strcat(utilizatori_online[contor++], argv[i]);
        }
    }
    return 0;
};
...
void adaugare_mesaj_in_baza_de_date (char *nume_utilizator1, char *nume_utilizator2, char *mesaj)
{
    ...
    strftime(data_trimitere, sizeof(data_trimitere), "%Y-%m-%d %H:%M:%S", localtime(&timp_curent));

    char comanda_sql[150];
    snprintf(comanda_sql, sizeof(comanda_sql),"INSERT INTO Mesaje (utilizator_sursa, utilizator_destinatie, mesaj, data_trimitere) VALUES('%s', '%s', '%s', '%s' );", nume_utilizator1, nume_utilizator2, mesaj, data_trimitere);
...
void adaugare_mesaj_offline_in_baza_de_date (char *nume_utilizator1, char *nume_utilizator2, char *mesaj)
{
    ...
    char data_trimitere_mesaj[20];
    time_t timp_curent = time(NULL);
    strftime(data_trimitere_mesaj, sizeof(data_trimitere_mesaj), "%Y-%m-%d %H:%M:%S", localtime(&timp_curent));

    char comanda_sql[150];
    snprintf(comanda_sql, sizeof(comanda_sql),"INSERT INTO Mesaje_offline (utilizator_sursa, utilizator_destinatie, mesaj, data_trimitere) VALUES('%s', '%s', '%s', '%s' );", nume_utilizator1, nume_utilizator2, mesaj, data_trimitere_mesaj);
